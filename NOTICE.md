# Third-party notices

Trolls' Printing Press bundles the following components:

- [typst-wasm](https://github.com/typst-wasm/typst-wasm), MIT License.
- The Typst compiler, Apache License 2.0.
- Libertinus Serif fonts, SIL Open Font License 1.1. Copyright © 2012–2024
  The Libertinus Project Authors. “Linux Libertine”, “Biolinum”, and “STIX
  Fonts” are Reserved Font Names.
- [markdown-it](https://github.com/markdown-it/markdown-it), MIT License.
- [pdf-lib](https://github.com/Hopding/pdf-lib), MIT License.

The complete corresponding license texts are distributed by the packages in
the source dependency tree and remain available from their linked upstream
repositories. Documents produced by Trolls' Printing Press are not subject to
the licenses of the bundled compiler or fonts.
